...
function bindEvents() {
  ...
  // Pull-to-refresh — endast på touchenheter
  if ('ontouchstart' in window) {
    let touchStartY = 0;
    const viewContainer = document.querySelector('.view-container');
    viewContainer.addEventListener('touchstart', e => { touchStartY = e.touches[0].clientY; }, { passive: true });
    viewContainer.addEventListener('touchend', e => {
      const deltaY = e.changedTouches[0].clientY - touchStartY;
      const activeView = document.querySelector('.view.active');
      if (deltaY > 80 && activeView && activeView.scrollTop <= 0 && !isRefreshing) {
        fetchCoins(true);
      }
    }, { passive: true });
  } else {
    // Desktop: dölj pull-indikatorn permanent
    document.getElementById('pull-indicator').style.display = 'none';
  }
}
